document.addEventListener("DOMContentLoaded", function () {
  let userId = null; // Biến để lưu ID của user

  // Function để lấy ID từ server
  function getUserId() {
      const apiUrl = '/connected_users'; // Đường dẫn API của bạn

      fetch(apiUrl)
          .then(response => response.json())
          .then(data => {
              console.log('User ID:', data.id);
              userId = data.id; // Lưu ID vào biến userId để sử dụng sau
          })
          .catch(error => {
  ...
  // Gọi hàm để lấy ID từ server khi trang vừa load
  getUserId();
  ...
  function displayAIMessage(message) {
    ...
    refreshButton.addEventListener('click', function () {
      ...
      showLoader();
      sendToServer(userId, lastUserMessage);
    });
  ...
  function sendToServer(userId, userMessage) {
      const apiUrl = '/send_message'; // Đường dẫn API của bạn để gửi tin nhắn
      ...
      .then(data => {
          ...
          hideLoader();
          displayAIMessage(data.response); // Hiển thị phản hồi từ AI (trả về từ server)
      })
      .catch((error) => {
